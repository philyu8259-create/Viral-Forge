...
- Latest end-to-end checks passed: `xcodebuild ... test` on iPhone 17 simulator for English locale generation, Chinese mock generation, result copy-pack export, result edit/save, poster export/assets flows, and the template library/detail workflow; backend `npm run check` and `npm run smoke:local` also passed.
